...
#define MARGIN 128
int is_pixelwhite(Image *img,int x,int y){
  if(img->data[y*img->width+x].r > MARGIN)return 0;
  return 1;
}
...
{
  if(argc != 2){
    fprintf(stderr, "Usage: program <manifest>\n");
    exit(1);
  }
  FILE *fp;
  fp = fopen(argv[1],"r");
  ...
  while(fgets(filename,sizeof(filename),fp)){
	...
	for(x=0;x<img->width;x+=2){
	  ...
	  for(dx=0;dx<2;dx++){
	    for(dy=0;dy<4;dy++){
	      data |= is_pixelwhite(img,x+dx,y+dy)<<(dx+dy*2);
	    }
	  }
	  printf("%c",data);
	}
